...
def setup(group):
    ...
    AD = AE.copy()
    for l in AD:
        l[0] = l[0] @ np.linalg.inv(Mj1)
        l[1] = l[1] @ np.linalg.inv(Mj2)
...
def param_encryption(params, base_enc_key, AE, group, g1):
    ...
    for i in range(params.shape[0]):
        ...
        for j in range(params.shape[1]):
            # x = int((int(alpha) * P1[i] @ X1 @ N1_inv)[j])
            # y = int((int(alpha) * P2[i] @ X2 @ N2_inv)[j])
            x = int(alpha) * int(P1[i][j])
            y = int(alpha) * int(P2[i][j])

            if x < -1:
                ct_p1.append((g1 ** -1) ** (-1 * x))
            else:
                ct_p1.append(g1 ** x)
            if y < -1:
                ct_p2.append((g1 ** -1) ** (-1 * y))
            else:
                ct_p2.append(g1 ** y)

        ciphertext.append((g1 ** alpha, ct_p1, ct_p2))
        # ciphertext.append((g1 ** alpha,
        #                    [(g1 ** int((int(alpha) * P1[i] @ X1 @ N1_inv)[j])) for j in range(params.shape[1])],
        #                    [(g1 ** int((int(alpha) * P2[i] @ X2 @ N2_inv)[j])) for j in range(params.shape[1])]))
